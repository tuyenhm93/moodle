...
define(['jquery', 'core/ajax', 'core/templates', 'core/notification',
    ...
    function($, ajax, templates, notification, str, url, Y, Fragment, ModalFactory) {
         ...
         * @param {JQuery} target the element (menu item) that was clicked
         */
        var showModuleEditingForm = function(moduleElement, html, javascript, cmid, module_type, sectionreturn){
            ...
            .done(function(modal){
                ...
                $(SELECTOR.SUBMITBUTTON2).click(function(e){
                    e.preventDefault();
                    onSubmitModuleEditingForm(moduleElement, cmid, module_type, sectionreturn, modal);
                });
                ...
                var validatorName = "validate_mod_" + module_type + "_mod_form";
         ...
         * @param {Object} modal
         */
        var onSubmitModuleEditingForm = function(moduleElement, cmid, module_type, sectionreturn, modal) {
            ...
            if(isValidated){
                ...
                $.when.apply($, promises).done(function(data) {
                    if(data.error) {
                        showModuleEditingForm(moduleElement, data.html, data.javascript, cmid, module_type, sectionreturn);
                    }else {
         ...
         * @param {Number} sectionreturn
         */
        var showModuleAddingForm = function(sectionElement, sectionid, html, javascript, module_type, sectionreturn){
            var jsNodes = $(javascript);
            // The first node always contains redefinition of require.
            // It should be removed, otherwise, function require will be overwritten by variable require.
            jsNodes.splice(0, 1);
            ...
            ModalFactory.create({
                ...
                .done(function(modal){
                    ...
                    $(SELECTOR.SUBMITBUTTON2).click(function(e){
                        e.preventDefault();
                        onSubmitModuleAddingForm(sectionElement, sectionid, module_type, modal, sectionreturn);
                    });
                    // Handle cancel submission
                    $(SELECTOR.CANCELBUTTON + ',' + SELECTOR.CANCELBUTTON2).on('click', function(e){
                        e.preventDefault();
         ...
         * @param {Number} sectionreturn
         */
        var onSubmitModuleAddingForm = function(sectionElement, sectionid, module_type, modal, sectionreturn){
            var isValidated = false;
            ...
            if(isValidated){
                ...
                $.when.apply($, promises).done(function(data) {
                    if(data.error) {
                        showModuleAddingForm(sectionElement, sectionid, data.html, data.javascript, module_type, sectionreturn);
                    }else {
